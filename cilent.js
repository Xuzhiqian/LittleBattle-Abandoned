Q.client_core = Q.core.extend({
	init:function() {
        	...
        	this.seq=0;
        	this.state={};
        	this.messages={text:[],length:5,tail:0,
	...
	this.mouse_click = function(e) {this.is_mouseclicked=true;};
	...
	client_render_message:function() {
		...
		for (var i=0;i<this.messages.length;i++) {
			...
			if (this.messages.text[index]!=undefined) {
				...
				ctx.fillText(this.messages.text[(i+this.messages.tail)%this.messages.length],map_width-160,50+i*20);
			}
		}
		ctx.restore();
	},
	...
	client_gameover:function(state) {
		if (state.id!=this.id) {
			this.messages.newmsg(state.id+ ' is killed');
			this.player_count = state.count;
		}
		else {
			this.game.socket.disconnect();
			Q.pauseGame();
			alert('You have been killed! Good luck next time!');
